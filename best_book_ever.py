...
my_url = 'https://www.goodreads.com/list/show/1.Best_Books_Ever?page=3'
...
for tr in trs:
    ...
    voted = scores[0].span.select("a:nth-child(3)")[0].text.replace(
        "people voted", "").replace("person voted", "").replace(",", ".").strip()
    # opening detail book
    ...
    book_total_pages = feature_layout.findAll(
        'p')[0].text.replace(" pages, Hardcover", '').replace(" pages, Paperback", '').replace(" pages, Kindle Edition", '').replace(" pages, ebook", '').replace(" pages, Mass Market Paperback", '').replace(" pages, Board book", '').replace(" pages, Unknown Binding", '')
    # book_published = feature_layout.findAll(
    #     'p')[1].text.replace("First published ", '').replace(",", '')
    book_date = feature_layout.findAll(
        'p')[1].text.replace("First published ", '').replace(",", '')
    is_valid_year = len(book_date.split()[2]) > 3
    book_published = ''
    if (is_valid_year):
        date_object = datetime.strptime(book_date, "%B %d %Y")
        book_published = date_object.strftime("%Y-%m-%d")
    elif (is_valid_year == False):
        book_published = 'null'
    ...
    is_author_born = author_right_layout.find(
        "div", {"class": "dataTitle"})
    author_born = ''
    if (is_author_born != None):
        author_born = author_right_layout.find(
            "div", {"class": "dataTitle"}).next_sibling.text.replace("\n", '').replace('in ', '').replace(',', '*').strip()
    elif (is_author_born == None):
        author_born = "null"
    # print(author_born)
    is_author_web = author_right_layout.find("div", {"class": "dataItem"})
    author_web = ''
    if (is_author_web != None):
        web_address = author_right_layout.find(
            "a", {"itemprop": "url"}).get("href")
        author_web = web_address
        if (web_address.startswith('/')):
            author_web = 'https://www.goodreads.com' + web_address
...
#     if (p.get('class') == None):
#         # only scrap until page 3
#         if (int(p.get("href").split('=')[1]) > 3):
#             quit()
